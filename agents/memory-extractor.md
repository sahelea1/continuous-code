...
```bash
# Note: the memory system path is configured via environment (CLAUDE_OPC_DIR or equivalent)
PYTHONPATH=. python3 scripts/core/extract_thinking_blocks.py \
  --jsonl "$JSONL_PATH" \
  --filter \
...
```bash
# Note: run from the memory system directory (path configured via environment)
# Example for a CORRECTION → ERROR_FIX
PYTHONPATH=. python3 scripts/core/store_learning.py \
